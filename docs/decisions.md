...
### API 프로세스에 커넥션 풀 도입 (2026-08-18)
- **이전 방식**: 요청마다 `create_connection()` 으로 새 커넥션. MySQL 이 Oracle Cloud VM 으로 옮겨간 뒤 WAN 경유 개설 비용이 **350ms** 로, `/builds` 응답 419ms 의 **90%** 를 차지했다
- **변경 이유**: 대부분의 시간을 일이 아니라 연결을 만드는 데 쓰고 있었다
- **결과**: DBUtils `PooledDB`. `get_connection()` 시그니처를 유지해 호출부 30여 곳 무수정. 단일 요청 419→52ms(**8.1배**), 처리량 93.6→373 RPS(**4.0배**), p95 455→104ms(**4.4배**)
- **풀 크기 20 의 근거**: 스윕 결과 P=20 에서 처리량·p95 가 **함께** 평평해진다(P=40 은 +3%, 변동 범위). 두 계산이 같은 값으로 수렴한다 — `threadpool 40 × 점유율 50% = 20`, `CPU 천장 378 RPS × 점유시간 55ms = 21`. P=40 에 동시 80명을 걸어도 커넥션 수요가 21.1 에서 멈춘다(= 풀이 더 이상 병목이 아니다)
- **트레이드오프**: 유휴 커넥션이 상시 5개(부하 후 20개) 생긴다. 재기동 후 첫 버스트의 **최악 지연 5190ms**(확장이 커넥션당 350ms 직렬) — p50·p95 는 정상이고 최악값만 1회 튄다. `ping=0` 이라 **staleness 대응이 미구현**이다
- **기각한 대안**: `ping=1`(대여 시 생존 검사) — 실측 50.1→100.0ms, **정확히 2배**. 드문 실패를 막으려 모든 요청을 2배 느리게 하는 거래다
- **적용 범위**: **API 프로세스 한정.** 크롤러·파이프라인은 `init_pool()` 을 부르지 않아 기존 동작 유지 — 2026-07-28 의 유휴 커넥션 사망 이력 때문이다(위 항목)
- **상세**: [adr_20260818_api_connection_pool.md](adr_20260818_api_connection_pool.md), 측정 원자료는 [../scripts/perf/RESULTS.md](../scripts/perf/RESULTS.md)
...
- **기각한 대안**: 커넥션 풀 — 이 장애는 커넥션 부족이 아니라 유휴 커넥션 사망이므로
  풀은 병을 키운다. **이 판단은 크롤러·파이프라인 한정으로 지금도 유효하다.**
  2026-08-18 에 API 프로세스에는 풀을 도입했으나(아래 항목) 크롤러는 그대로 두었다 —
  크롤러는 커넥션을 거의 안 쓰면서(각 크롤러 첫머리 1회) 유휴 시간만 길어 정반대 조건이다
- **정정 (2026-08-18)**: 원문의 "실제로 문제를 푸는 건 풀이 아니라 체크아웃 시 ping이다" 는
  **실측으로 반증됐다.** `ping=1` 은 대여마다 `COM_PING` 왕복을 더해 50.1ms → 100.0ms
  (정확히 2배)가 된다. 비용을 재보지 않은 판단이었다. staleness 대응은 ping 이 아니라
  유휴 수명 제한이나 예외 기반 재시도로 가야 한다(미구현)
