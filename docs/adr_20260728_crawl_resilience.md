...
### 크롤링 주체를 GitHub Actions → Oracle Cloud VM으로 이전

**보류. 현재로선 불필요하다.**

컴퓨존이 막는 대상은 GitHub Actions 러너(Azure 대역) IP이므로, 실행 주체를 VM cron으로
옮기면 나가는 IP가 바뀌어 차단을 피할 여지가 있다. 덤으로 MySQL이 같은 호스트가 되어
원격 커넥션 문제 자체가 사라진다. 그럼에도 지금은 하지 않는다.

- 이번 장애의 실제 exit 1 원인(ⓐⓑⓒ)은 해소됐다. 이제 컴퓨존이 막혀도 나머지 두 사이트와
  파이프라인은 정상 동작하고, 워크플로도 성공한다
- 남는 손실은 "그날 컴퓨존 가격만 누락"이며, 최근 12회 중 2회 발생한 간헐적 현상이다
- 그 손실을 막자고 GitHub Actions의 관리 편의(실행 로그·재실행 UI, push 시 자동 반영,
  인프라 무관리)를 포기하는 것은 균형이 맞지 않는다

**이전한다면 3개 사이트 전부**를 옮긴다. 컴퓨존만 떼어내면 얻는 것은 한 사이트인데
스케줄·배포·알림 관리 지점이 2배가 되고, 크롤링 이후 적재→변환→집계로 이어지는 한
프로세스를 두 군데로 나눠 설계해야 한다.

**선행 확인 사항**: Oracle VM의 리전. 한국 리전이면 국내 IP라 차단 회피 가능성이 높지만,
해외 리전이면 GitHub 러너와 같은 처지여서 옮기는 의미가 없다. 이전을 검토할 때 가장 먼저
확인한다.

**재검토 시점**: 컴퓨존 실패율이 뚜렷이 상승할 때. 실패는 `crawl_failures`에 기록되고
Slack으로 알려지므로 몇 주면 판단 근거가 쌓인다.
...
### 실제 장애 조건에서의 실증 (run 30350384888, 2026-07-28 10:21 KST)

수정 브랜치로 수동 실행한 결과, **컴퓨존이 같은 4개 대상에 대해 동일하게 무응답인
상태가 재현됐고**(`connect timeout=5.0`으로 실패), 파이프라인은 정상 완료했다.

| | 08:39 실행 (수정 전) | 10:21 실행 (수정 후) |
|---|---|---|
| 컴퓨존 구간 | 5분 6초 | **1분 5초** |
| 대상당 소요 | 77초 | **약 16초** |
| 견적왕 | `(2006)` 사망 | **`kjwwang: 1건` 정상** |
| 다나와 1건 | 유실 | **적재됨** |
| 총 적재 | 0건 | **2건** |
| 종료 코드 | exit 1 | **exit 0** |

이후 단계도 모두 정상: Raw 2건 적재 → Staging 2건 변환 → 레이어 정합성 손실률 0.0%
→ Slack 알림 전송(status=200) → Analytics 집계 완료.

ⓐⓑⓒ 세 결정이 모두 실제로 동작함이 로그로 확인됐다. 단위 테스트만으로는 얻을 수 없는
검증이며, 컴퓨존 차단이 간헐적이라 재현 조건을 의도적으로 만들기 어렵다는 점에서
운이 따른 확인이기도 하다.

### 이번 결정으로 닫지 않은 것 (해야 할 일이 아니라 선택지)

- **컴퓨존 IP 차단 자체** — 러너 측 코드로는 해결할 수 없다. 다만 이제 차단되어도
  파이프라인은 정상 동작하므로 대응하지 않아도 무방하다. VM 이전은 §3 참조 (현재 보류)
- **API 프로세스의 커넥션 재사용** — 크롤링과 별개 프로세스·별개 문제. 실제 응답시간
  측정 후 필요성을 판단한다
