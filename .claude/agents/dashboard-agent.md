...
description: Dash/Plotly 웹 대시보드 개발·수정 전문 에이전트. 레이아웃, 콜백, Plotly 차트, MySQL 쿼리 최적화를 담당한다.
model: opus
---
...
- MySQL 쿼리 최적화: `src/dashboard/data_access/mysql_queries.py`
- 대시보드 헬퍼 함수: `src/dashboard/helpers.py`

## 작업 원칙

1. 기존 레이아웃 파일 구조(`layouts/overview.py`, `prices.py`, `trends.py` 등)를 따른다
2. MySQL 쿼리는 Analytics(ans_) 레이어만 조회한다 (Raw/Staging 직접 조회 금지)
...
- MySQL 연결 실패 시 사용자에게 Docker 서비스 상태 확인을 안내한다
