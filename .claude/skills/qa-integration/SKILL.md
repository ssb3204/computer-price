---
name: qa-integration
description: "통합 테스트 및 데이터 정합성 검증 스킬. pytest 실행, 크롤러 출력 검증, 파이프라인 레이어 간 손실률 확인, API 응답 검증 등 테스트·검증 작업 시 반드시 이 스킬을 사용. 모듈 완성 직후 점진적으로 실행."
---

# QA 통합 검증 스킬

크롤러 → MySQL → API 전 레이어의 통합 테스트와 데이터 정합성 검증 지침.
